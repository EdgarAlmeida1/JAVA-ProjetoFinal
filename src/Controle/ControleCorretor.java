        ...
        for (int i = 0; i < imovel.size(); i++) {
            ...
            for (int j = 0; j < visitas.size(); j++) {
                ...
                if (visitas.get(j).getCorretor().getCreci().equals(corretor.getCreci())
                    ...
                    resultado += ("Imovel " + imovel.get(i).getCodigo() + " foi visitado pelo comprador " + visitas.get(j).getComprador().getCpf()
                            + "\ne pelo corretor " + corretor.getCreci() + " em " + visitas.get(j).getData().getTime()) + "\n";
